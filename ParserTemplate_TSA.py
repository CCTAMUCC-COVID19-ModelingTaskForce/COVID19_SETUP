...
# ------------------------------------------------------------------------
# Globals
##COUNTIES_PLACEHOLDER
##NAME_PLACEHOLDER
TSA = parserName.replace("TSA-", "")
...
def parse():
    ...
    dfHospitalizedFile = "../COVID19_SETUP/out/texas_hosps_tsa_severe.csv"
    dfIcuFile = "../COVID19_SETUP/out/texas_hosps_tsa_critical.csv"
    ...
    # Read hospitalization data (severe <-> medsurge <-> hospital beds)
    dfHosps = pd.read_csv(dfHospitalizedFile)
    dfHosps.columns = [c.replace('severe_', '2020-').replace('severe', '2020-').lower() for c in dfHosps.columns.to_list()]
    # Filter by TSA
    dfHosps = dfHosps[dfHosps['tsa_id'] == TSA]

    # Read hospitalization data (critical <-> ICU)
    dfIcu = pd.read_csv(dfIcuFile)
    dfIcu.columns = [c.replace('critical', '2020-').replace('critical', '2020-').lower() for c in dfIcu.columns.to_list()]
    # Filter by TSA
    dfIcu = dfIcu[dfIcu['tsa_id'] == TSA]
    ...
    for t in df['time']:
        ...
        try:
            prevHosp = dfHosps[t].sum()
            df.loc[df['time'] == t, 'hosps'] = prevHosp
        except:
            df.loc[df['time'] == t, 'hosps'] = prevHosp # Repeat
        try:
            prevIcu = dfIcu[t].sum()
            df.loc[df['time'] == t, 'icus'] = prevIcu
        except:
            df.loc[df['time'] == t, 'icus'] = prevIcu # Repeat
    ...
    for t in df['time']:
        regions[parserName].append([t, int(df.loc[df['time'] == t, 'cases']),
                 ...
                 int(df.loc[df['time'] == t, 'hosps']),
                 int(df.loc[df['time'] == t, 'icus']),
                 None,
                ])
