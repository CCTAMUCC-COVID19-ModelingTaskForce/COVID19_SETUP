...
import { selectRunParams } from '../state/scenario/scenario.selectors'
...
export function collectTotals(trajectory: SimulationTimePoint[], ages: string[], P: ModelParams): ExportedTimePoint[] {
  const res: ExportedTimePoint[] = []

  trajectory.forEach((d, day) => {
    const prevDay = day > 7 ? day - 7 : 0
    const prevDayTen = day > 10 ? day - 10 : 0
    ...
    Object.keys(tp.current).forEach((k) => {
      if (k === 'exposed') {
        ...
        if (prevDayTen > 0) {
          tp.current.infected.total = 
            (P.populationServed - trajectory[prevDayTen].current.susceptible.reduce((a, b) => a + b, 0)) / 5.6
        }
      } else if (k === 'dailyNewInfected') {
        if (prevDayTen > 0) {
        ...
        if (prevDayTen - 7 > 0) {
          tp.current.weeklyNewInfected.total = (
            (
              (P.populationServed - trajectory[prevDayTen].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 1].current.susceptible.reduce((a, b) => a + b, 0))
            ) +
            (
              (P.populationServed - trajectory[prevDayTen - 1].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 2].current.susceptible.reduce((a, b) => a + b, 0))
            ) + 
            (
              (P.populationServed - trajectory[prevDayTen - 2].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 3].current.susceptible.reduce((a, b) => a + b, 0))
            ) + 
            (
              (P.populationServed - trajectory[prevDayTen - 3].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 4].current.susceptible.reduce((a, b) => a + b, 0))
            ) + 
            (
              (P.populationServed - trajectory[prevDayTen - 4].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 5].current.susceptible.reduce((a, b) => a + b, 0))
            ) + 
            (
              (P.populationServed - trajectory[prevDayTen - 5].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 6].current.susceptible.reduce((a, b) => a + b, 0))
            ) + 
            (
              (P.populationServed - trajectory[prevDayTen - 6].current.susceptible.reduce((a, b) => a + b, 0))
                - (P.populationServed - trajectory[prevDayTen - 7].current.susceptible.reduce((a, b) => a + b, 0))
            ) 
          ) / 5.6
        }
      } else {
