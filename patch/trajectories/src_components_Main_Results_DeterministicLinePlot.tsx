...
export function DeterministicLinePlotDiconnected({
  ...
  const [newEmpiricalDailyCases] = computeNewEmpiricalCases(1, 'cases', caseCountsData)
  ...
  const hasObservations = {
    ...
    [DATA_POINTS.ObservedNewDailyCases]: newEmpiricalDailyCases && newEmpiricalDailyCases.some((d) => d),
  ...
  const observations =
    caseCountsData?.map((d, i) => ({
      ...
      newDailyCases: enabledPlots.includes(DATA_POINTS.ObservedNewDailyCases) ? newEmpiricalDailyCases[i] : undefined,
    ...
    <div className="w-100 h-100" data-testid="DeterministicLinePlot">
      <ReactResizeDetector handleWidth handleHeight refreshRate={300} refreshMode="debounce">
        {({ width }: { width?: number }) => {
          ...
          const ms2week = (86400000 * 7)
          const xMax_ = plotXMax > 0 ? plotXMax : tMax
          const xMax = xMax_ == tMax ? tMax : tMin + xMax_ * ms2week
          const xMin = plotXMin < plotXMax 
            ? tMin + plotXMin * ms2week 
            : tMin + (plotXMax - 1) * ms2week
          const xDataDomain = [xMin, xMax]

          const yMax = plotYMax > 0 ? plotYMax : yDataMax
          const yMin_ = plotYMin <= yMax ? plotYMin : yMax - 10
          const yMin = isLogScale && yMin_ == 0 ? 1 : yMin_
          const yDataDomain = [yMin, yMax * 1.1] 
